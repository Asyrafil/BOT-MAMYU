...
async function sendMainMenuButtons(sock, to, text){
    try{
        await sock.sendMessage(to, {
            ...
            buttons: [
                ...
                { buttonId: "po", buttonText: { displayText: "🛒 PO" }, type: 1 },
                { buttonId: "ongkir", buttonText: { displayText: "🚚 Ongkir" }, type: 1 },
                { buttonId: "!status", buttonText: { displayText: "📊 Status Bot" }, type: 1 }
            ],
            headerType: 1
        })
    }catch(error){
        console.log("Interactive button fallback:", error?.message || error)
        await sock.sendMessage(to, {
            text: `${text}

Ketik: menu | po | ongkir | !status`
        })
    }
}
...
Ada yang bisa saya bantu hari ini?`
    ...
    if(!SHOP_CONFIG.openDays.includes(today)){
        await sendMainMenuButtons(sock, from, SHOP_CONFIG.closedMessage)
        return
    }
    ...
    if(askCancel && isInQueue(sender)){
        ...
        await sendMainMenuButtons(
            sock,
            from,
            "❌ Oke kak, PO kamu dibatalkan. Kamu bisa PO lagi kapan saja."
        )
        return
    }
    ...
    if(isInQueue(sender)){
        ...
        if(!nama || !alamat || !hp || !porsi){
            await sendMainMenuButtons(sock, from, `⚠️ Format PO belum lengkap atau ada yang kosong!
...
- Di luar Batam Center: 13k`)
        ...
        try{
            ...
            await sendMainMenuButtons(sock, from, `✅ Terima kasih kak 💕
...
Selamat datang di Mamyu.bykar🥰
Silahkan pilih tombol di bawah atau ketik menu.`
        )
        return
    }
...
Ketik *"PO"*, *"PESAN"* Untuk Melanjutkan List Pesanan ‼️‼️`
        ...
        await sendMainMenuButtons(
            sock,
            from,
            "Silahkan pilih tombol di bawah untuk lanjut ya kak."
        )
        return
    }

    if(askOngkir){
        await sendMainMenuButtons(sock, from, `🚚 ONGKIR :
...
Ketik *"PO"*, *"PESAN"* Untuk Melanjutkan List Pesanan ‼️‼️`)
    ...
    if(askPengantaran){
        await sendMainMenuButtons(sock, from, `🚚 PENGANTARAN :
...
Ketik *"PO"*, *"PESAN"* Untuk Melanjutkan List Pesanan ‼️‼️`)
    ...
    if(askPO){
        addToQueue(sender)
        await sendMainMenuButtons(sock, from, `Hai kak 👋
...
- Di luar Batam Center: 13k`)
    ...
    if(normalizedText.startsWith(PREFIX)){
        ...
        if(command === "jid"){
            await sendMainMenuButtons(sock, from, `JID: ${chatMetadata.id}`)
            return
        }
        ...
        if(command === "status"){
            ...
            await sendMainMenuButtons(sock, from, `📊 *STATUS BOT*
